...
`schema.sql` is **generated** — the source of truth is split by purpose under `supabase/sql/` (entry point: `supabase/sql/index.sql`). Edit the partials, then run `npm run build:schema` to regenerate `schema.sql`. `npm run build` runs this automatically via `prebuild`.
...
schema.sql            — Database DDL (generated; idempotent). Regenerated by `npm run build:schema`.
supabase/sql/         — Source partials split by purpose (groups, settings, stats, …). index.sql lists load order.
supabase/functions/   — Edge Functions (click tracking)
scripts/build-schema.mjs — Concatenates supabase/sql/*.sql into schema.sql
